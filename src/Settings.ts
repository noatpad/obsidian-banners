  ...
  display() {
    ...
    new Setting(containerEl)
      ...
      .addText((text) => {
        ...
        const shown_height_uom = height_uom === 'px' ? '' : height_uom;
        text.setValue(`${height || DEFAULT_VALUES.height}${shown_height_uom}`);
        text.setPlaceholder(`${DEFAULT_VALUES.height}`);
        text.onChange(async (val) => {
          const [height] = String(val || DEFAULT_VALUES.height || '').match(/[0-9]+/);

          const offset = val.indexOf(height) + height.length;
          const height_uom = val.slice(offset);

          return this.saveSettings({
            height: parseInt(height) || null,
            height_uom: height_uom || 'px'
          });
        });
      });
    ...
    new Setting(containerEl)
      ...
      .addToggle((toggle) =>
        toggle
          ...
          .onChange(async (val) =>
            this.saveSettings({ showInInternalEmbed: val }, { reloadSettings: true, refreshViews: true })
          )
      );
    ...
    if (this.plugin.settings.showInInternalEmbed) {
      new Setting(containerEl)
        ...
        .addText((text) => {
          ...
          text.onChange(async (val) =>
            this.saveSettings({
              internalEmbedHeight: val ? parseInt(val) : null
            })
          );
        });
    }
    ...
    if (this.plugin.settings.showInPreviewEmbed) {
      new Setting(containerEl)
        ...
        .addText((text) => {
          ...
          text.onChange(async (val) =>
            this.saveSettings({
              previewEmbedHeight: val ? parseInt(val) : null
            })
          );
        });
    }
    ...
    new Setting(containerEl)
      ...
      .setDesc(
        createFragment((frag) => {
          ...
          frag.createEl('code', {
            text: DEFAULT_VALUES.frontmatterField
          });
          ...
          frag.appendText(', ');
          frag.createEl('code', {
            text: `${DEFAULT_VALUES.frontmatterField}_x`
          });
          frag.appendText(', ');
          frag.createEl('code', {
            text: `${DEFAULT_VALUES.frontmatterField}_y`
          });
    ...
    const settingHIA = new Setting(containerEl).setName('Horizontal alignment').setDesc(
      createFragment((frag) => {
    ...
    settingHIA.addDropdown((dd) =>
      dd
        .addOptions(ICON_HORIZONTAL_OPTIONS)
        .setValue(iconHorizontalAlignment)
        .onChange(async (val: IconHorizontalOption) =>
          this.saveSettings({ iconHorizontalAlignment: val }, { reloadSettings: true, refreshViews: true })
        )
    );

    // Vertical icon alignment
    const settingVIA = new Setting(containerEl).setName('Vertical alignment').setDesc(
      createFragment((frag) => {
    ...
    settingVIA.addDropdown((dd) =>
      dd
        .addOptions(ICON_VERTICAL_OPTIONS)
        .setValue(iconVerticalAlignment)
        .onChange(async (val: IconVerticalOption) =>
          this.saveSettings({ iconVerticalAlignment: val }, { reloadSettings: true, refreshViews: true })
        )
    );
    ...
    new Setting(containerEl)
      ...
      .addText((text) => {
        ...
        text.onChange(async (val) =>
          this.saveSettings({
            localSuggestionsLimit: val ? parseInt(val) : null
          })
        );
      });
