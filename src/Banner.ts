...
const buildBanner = (
  ...
  img.onerror = () => {
    messageBox.innerHTML = `<p>Error loading banner image! Is the <code>${plugin.getSettingValue(
      'frontmatterField'
    )}</code> field valid?</p>`;
  ...
  if (canDrag) {
    img.classList.toggle(
      'draggable',
      plugin.settings.bannerDragModifier === 'none' || plugin.holdingDragModKey
    );

    // Image drag
    const imgDragStart = (e: MTEvent) =>
      handleDragStart(e, dragData, plugin.holdingDragModKey);
  ...
  // Prepare listeners
  listeners.forEach(({ target, ev, func }) =>
